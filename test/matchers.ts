...
import type { GeoJSONGeometry } from '../src';

// Add custom matchers for geometry objects
expect.extend({
  toMatchWkt(received: { toWkt: () => string }, expected: string) {
  ...
  toMatchWkb(received: { toWkb: () => Buffer }, expected: string) {
  ...
  toMatchEwkt(received: { toEwkt: () => string }, wkt: string, srid: number = 4326) {
  ...
  toMatchEwkb(received: { toEwkb: () => Buffer }, expected: string) {
  ...
  toMatchTwkb(received: { toTwkb: () => Buffer }, expected: string) {
  ...
  toMatchGeoJSON(received: { toGeoJSON: () => GeoJSONGeometry }, expected: GeoJSONGeometry) {
...
interface CustomMatchers {
    ...
    toMatchGeoJSON(expected: GeoJSONGeometry): void;
}
